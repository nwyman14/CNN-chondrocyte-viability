...
def cnn_model(X_train, y_train, kernel_size, nb_filters, channels, nb_epoch, batch_size, nb_classes, nb_gpus):
	...
	"""
	model = Sequential()
	model.add(Conv2D(16, (5, 5), input_shape=(img_rows, img_cols, channels), activation='relu'))
...
def save_model(model, score, model_name):
	...
	"""this part only allows the better models to get saved, but that's not really necessary right now"""
	#if score >= 0.75:
	#	print("Saving Model")
