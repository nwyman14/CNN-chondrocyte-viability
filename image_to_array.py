...
def change_image_name(df, column):
    ...
    """
    return [i + '.png' for i in df[column]]
...
def convert_images_to_arrays_train(file_path, df):
    ...
    for img in lst_imgs:
        try:
            if(np.array(Image.open(file_path + img)).shape==(160,160,3)):
                outarray.append(np.array(Image.open(file_path + img)))
            else:
                outarray.append(np.array(Image.open(file_path + img))[:,:,:3])
        except:
            pass
    return np.array(outarray, ndmin=3)
...
if __name__ == '__main__':
    ...
    labels = pd.read_csv("/data/trainLabels_cells.csv")

    print("Writing Train Array")
    X_train = convert_images_to_arrays_train('/data/train/', labels)
